...
public class PassengerRepositoryImpl extends BaseRemoteRepository implements PassengerRepository {
    ...
    @Override
    public void createPassenger(RequestResponseListener listener, int userId, Map<String, String> param) {
        Call<DataResult<Penumpang>> call = ((IPassengerEndPoints) endPoint).createPenumpang(userId, param);
        execute(call, listener);
    }

    @Override
    public void updatePassenger(RequestResponseListener listener, int userId, int idPenumpang, Map<String, String> param) {
        Call<DataResult<Penumpang>> call = ((IPassengerEndPoints) endPoint).updatePenumpang(userId, idPenumpang, param);
        execute(call, listener);
    }

    @Override
    public void deletePassenger(RequestResponseListener listener, int userId, int idPenumpang, Map<String, String> param) {
        Call<DataResult> call = ((IPassengerEndPoints) endPoint).deletePenumpang(userId, idPenumpang, param);
        execute(call, listener);
    }

    @Override
    public void listPassenger(RequestResponseListener listener, int userId, Map<String, String> param) {
